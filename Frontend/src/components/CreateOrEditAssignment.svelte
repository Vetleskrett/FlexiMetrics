<script lang="ts">
    ...
    import { CollaborationType, Semester, type AnalyzerFieldType, type AssignmentField, type NewAssignmentField } from 'src/types';
    ...
    import { Combobox, DatePicker } from 'bits-ui';
    ...
    export let asssignmentName: string = "", dueDate: string = "", collaberationType: CollaborationType = CollaborationType.Teams, draft: boolean = false, fields: NewAssignmentField[] = [], redirect: string, edit: boolean = false;
    export let submitFunction: (asssignmentName: string, dueDate: string, collaberationType: CollaborationType, draft: boolean, fields: NewAssignmentField[]) => void;
</script>

<form method="POST" on:submit|preventDefault={() => submitFunction(asssignmentName, toUTC(dueDate), collaberationType, draft , fields)}>
    <Card.Root class="m-auto mt-16 w-[700px] overflow-hidden p-0">
        <Card.Header class="mb-6 flex flex-row items-center justify-between">
            ...
            <div class="flex items-center font-bold">
                ...
                <Checkbox id="draftCheckbox"></Checkbox>
            </div>
        </Card.Header>
